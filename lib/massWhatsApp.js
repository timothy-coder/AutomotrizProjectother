...
function personalizeText(text, recipientName) {
  ...
  const safeName = String(recipientName || "").trim() || "cliente";

  return raw
    .replace(/\[\s*nombre\s+de\s+cliente\s*\]/gi, safeName)
    .replace(/\[\s*nombre\s+del\s+cliente\s*\]/gi, safeName)
    .replace(/\{\{\s*nombre_cliente\s*\}\}/gi, safeName)
    .replace(/\{\s*nombre_cliente\s*\}/gi, safeName);
}
...
export async function queueCampaignRecipient({
  ...
  const personalizedMessage = personalizeText(messageText, recipient?.recipient_name);
  const personalizedGreeting = personalizeText(content?.greeting || "", recipient?.recipient_name);
  const personalizedBody = personalizeText(content?.body || "", recipient?.recipient_name);
  const personalizedClosing = personalizeText(content?.closing || "", recipient?.recipient_name);
  ...
  const rawImageUrl = content?.image_url || null;
  const mediaBaseUrl = (process.env.MEDIA_PUBLIC_BASE_URL || process.env.APP_BASE_URL || "").replace(/\/$/, "");
  const absoluteImageUrl = rawImageUrl
    ? rawImageUrl.startsWith("http")
      ? rawImageUrl
      : `${mediaBaseUrl}${rawImageUrl}`
    : null;

  // ── Plantilla aprobada de Meta ──────────────────────────────────
  const isApprovedTemplate = content?.template_source === "aprobada" && content?.whatsapp_template_name;
  let whatsappTemplate = null;

  if (isApprovedTemplate) {
    const rawVars = Array.isArray(content.whatsapp_template_variables)
      ? content.whatsapp_template_variables
      : [];

    const resolvedVars = rawVars.map((v) => {
      const val = String(v || "").trim();
      if (val === "{{nombre_cliente}}" || val === "[nombre_cliente]" || val === "") {
        return personalizeText("{{nombre_cliente}}", recipient?.recipient_name);
      }
      return personalizeText(val, recipient?.recipient_name);
    });

    whatsappTemplate = {
      name: content.whatsapp_template_name,
      language: { code: content.whatsapp_template_language || "es" },
      components: resolvedVars.length
        ? [
          {
            type: "body",
            parameters: resolvedVars.map((text) => ({ type: "text", text })),
          },
        ]
        : [],
    };
  ...
  const payload = {
    ...
    // modo de envío — n8n lee este campo para decidir qué API usar
    template_source: content?.template_source || "libre",
    template_mode: content?.template_mode || "texto",
    // si template_source === "aprobada", este objeto tiene todo lo que n8n necesita
    whatsapp_template: whatsappTemplate,
    template_content: {
      image_url: absoluteImageUrl,
      greeting: personalizedGreeting || null,
      body: personalizedBody || null,
      closing: personalizedClosing || null,
    },
...
export async function dispatchCampaign(campaignId) {
  ...
  // Disparo async del outbox — no bloqueante, fire-and-forget
  if (queued > 0) {
    const outboxUrl = process.env.OUTBOX_PROCESS_URL;
    const outboxSecret = process.env.CONVERSATIONS_OUTBOX_SECRET;
    if (outboxUrl) {
      fetch(outboxUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(outboxSecret ? { "x-conversations-outbox-secret": outboxSecret } : {}),
        },
        body: JSON.stringify({ limit: Math.min(queued, 50) }),
      }).catch(() => {});
    }
  }
