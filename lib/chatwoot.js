...
export async function sendMessageWithAttachment(conversationId, { content = "", file, isPrivate = false } = {}) {
  const { baseUrl, headers } = getConfig();
  ...
  let res;
  try {
    res = await fetch(`${baseUrl}/conversations/${conversationId}/messages`, {
      method: "POST",
      headers: { api_access_token: headers.api_access_token },
      body: form,
    });
  } catch (networkErr) {
    throw new Error(`Chatwoot network error — POST /conversations/${conversationId}/messages: ${networkErr.message}`);
  }
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(`Chatwoot POST /conversations/${conversationId}/messages → ${res.status}: ${text}`);
  }
  return res.json().catch((err) => {
    throw new Error(`Chatwoot POST /conversations/${conversationId}/messages → non-JSON response: ${err.message}`);
  });
}
...
// Estado
export async function updateConversationStatus(conversationId, status, snoozedUntil = null) {
  const body = { status };
...
export async function searchConversations(query) {
  return chatwootFetch(`/search?q=${encodeURIComponent(query)}&page=1`);
}
...
export async function getInboxes() {
  return chatwootFetch("/inboxes");
}

// Plantillas de mensaje — las plantillas vienen dentro del objeto inbox en este Chatwoot
export async function getMessageTemplates(inboxId) {
  const data = await chatwootFetch(`/inboxes/${inboxId}`);
  return Array.isArray(data?.message_templates) ? data.message_templates : [];
}
