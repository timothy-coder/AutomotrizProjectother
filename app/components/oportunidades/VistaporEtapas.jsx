...
function renderTooltipContent(item) {
  return (
    <div className="space-y-1 text-sm">
      <div><span className="font-semibold">Código:</span> {item?.oportunidad_id || "-"}</div>
      <div><span className="font-semibold">Cliente:</span> {item?.cliente_name || "-"}</div>
      <div><span className="font-semibold">Vehículo:</span> {item?.modelo_name || "-"} {item?.marca_name ? `- ${item.marca_name}` : ""}</div>
      <div><span className="font-semibold">Etapa:</span> {item?.etapa_name || "-"}</div>
      <div><span className="font-semibold">Asignado a:</span> {item?.asignado_a_name || "Sin asignar"}</div>
      <div><span className="font-semibold">Fecha:</span> {item?.fecha_agenda || "-"}</div>
      <div><span className="font-semibold">Hora:</span> {getHoraLabel(item?.hora_agenda) || "-"}</div>
      {item?.detalle ? (
...
export default function VistaPorEtapas({ rows, onOpenOportunidad }) {
  ...
  const rowsConEtapa = useMemo(() => {
    return rows.filter((r) => r?.etapasconversion_id);
  }, [rows]);
                        ...
                        items.map((item) => (
                          <Tooltip key={item.id}>
                            <TooltipTrigger asChild>
                              ...
                              >
                                {String(item.oportunidad_id || "").replace("OP-", "")}
                              </button>
                            </TooltipTrigger>
