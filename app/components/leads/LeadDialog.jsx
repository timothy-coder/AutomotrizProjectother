...
export default function LeadDialog({
  ...
  const permViewAll = hasPermission(permissions, "leads", "viewall");
  ...
  const baseApi = "/api/leads";
  const recordLabel = "lead";
  const recordLabelCap = "Lead";

  // ==================== CARGAR DETALLES ====================
  async function loadDetalles(leadId) {
    try {
      setLoadingDetalles(true);
      const res = await fetch(
        `${baseApi}/${leadId}/detalles`,
        { cache: "no-store" }
      );
  ...
  // ==================== CARGAR ACTIVIDADES ====================
  async function loadActividades(leadId) {
    try {
      setLoadingActividades(true);
      const res = await fetch(
        `/api/actividades-oportunidades?oportunidad_id=${leadId}`,
        { cache: "no-store" }
      );
      const data = await res.json();
  ...
  // ==================== VALIDAR LEAD ACTIVO ====================
  async function verificarLeadActivo(clienteId) {
    try {
      const res = await fetch(
        `/api/leads?cliente_id=${clienteId}&limit=100`,
        { cache: "no-store" }
      );
      const data = await res.json();
      ...
      // Filtrar leads activos (no cerrados)
      const leadsActivos = leads.filter((ld) => {
        const etapaId = ld.etapasconversion_id;
        return !etapasCerradas.some((e) => String(e.id) === String(etapaId));
      });

      return leadsActivos.length > 0 ? leadsActivos[0] : null;
    } catch (error) {
      console.error("Error verificando lead activo:", error);
      return null;
    }
  }
  ...
  useEffect(() => {
        ...
        if (lead) {
          ...
          setForm({
            ...
            cliente_id: lead.cliente_id
              ? String(lead.cliente_id)
              : "",
            origen_id: lead.origen_id
              ? String(lead.origen_id)
              : "",
            suborigen_id: lead.suborigen_id
              ? String(lead.suborigen_id)
              : "",
            detalle: lead.detalle || "",
            etapasconversion_id: lead.etapasconversion_id
              ? String(lead.etapasconversion_id)
              : "",
            asignado_a: permViewAll
              ? lead.asignado_a
                ? String(lead.asignado_a)
                : "sin-asignar"
              : "sin-asignar",
          });
          ...
          loadActividades(lead.id);
          loadDetalles(lead.id);
        } else {
          setMode("new");
          setForm({
            ...EMPTY_FORM,
    ...
    loadInitialData();
  }, [open, currentUserId, lead, permViewAll]);
  ...
  function handleAgregarDetalleLocal() {
    ...
    if (isNew && detallesLocales.length >= 1) {
      toast.warning("Solo puedes agregar un detalle de agenda en nuevos leads");
      return;
    }
  ...
  async function handleGuardarDetalle() {
    ...
    try {
      ...
      if (editingDetalleId) {
        res = await fetch(
          `${baseApi}/${lead.id}/detalles/${editingDetalleId}`,
          {
            method: "PUT",
            headers: {
        ...
        res = await fetch(`${baseApi}/${lead.id}/detalles`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  async function handleEliminarDetalle(detalleId) {
    ...
    try {
      ...
      const res = await fetch(
        `${baseApi}/${lead.id}/detalles/${detalleId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleGuardarActividad() {
    ...
    try {
      ...
      const res = await fetch("/api/actividades-leads", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const leadActivo = await verificarLeadActivo(form.cliente_id);

      if (leadActivo) {
        const numeroLead =
          leadActivo.oportunidad_id || `LD-${leadActivo.id}`;
        
        toast.error(
          `No se puede crear un nuevo lead. Ya existe un lead activo para este cliente: ${numeroLead}`
        );
        return;
      }
      ...
      if (actividadesLocales.length > 0) {
        for (const actividad of actividadesLocales) {
          try {
            await fetch("/api/actividades-leads", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            if (actividad.etapasconversion_id) {
              await fetch(`${baseApi}/${leadId}/cambiar-etapa`, {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
                },
      ...
      toast.success(
        `${recordLabelCap} creado: ${numeroLeadNuevo}`
      );
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const res = await fetch(`${baseApi}/${lead.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const isCreating = guardandoLead || loading || guardandoDetalle || guardandoActividad;
                ...
                <div className="flex gap-2">
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      Abrir detalle completo de este lead
                    </TooltipContent>
                  </Tooltip>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right">
                      Selecciona el cliente para este lead
                    </TooltipContent>
                  </Tooltip>
                </div>
                {isNew ? (
              ...
              <div className="md:col-span-2">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right">
                      Descripción detallada del lead
                    </TooltipContent>
                  </Tooltip>
                </div>
                <textarea
              ...
              <div className="md:col-span-2 space-y-4 pt-6 border-t-2 border-slate-200">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 space-y-3 border border-green-200">
                  ...
                  {/* FORMULARIO DETALLE - SOLO NUEVOS O EDITAR CON BOTÓN REPROGRAMAR */}
