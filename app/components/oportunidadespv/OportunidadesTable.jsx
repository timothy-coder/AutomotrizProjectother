...
export default function OportunidadesTable({
  ...
  const handleVerOportunidad = (oportunidad) => {
    const route = type === "leadspv" ? "/leads" : "/oportunidades";
    router.push(`${route}/${oportunidad.id}`);
  };
  ...
  const columns = useMemo(
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-2">
            ...
            {canAssign && (
              <Button className="text-black" variant="outline" size="sm" onClick={() => onAssign(row.original)}>
    ...
    [canEdit, canAssign, onEdit, onAssign, type]
  );
              ...
              {loading ? (
                ...
                table.getRowModel().rows.map((row) => {
                  ...
                  const colores = getColorEstadoTiempo(
                    minutosRestantes,
                    row.original?.etapasconversionpv_id
                  );
