// app/api/etapasconversionpv/route.js
...
export async function POST(req) {
  try {
    ...
    const [maxRows] = await db.query(
      "SELECT COALESCE(MAX(sort_order), 0) AS maxOrder FROM etapasconversionpv"
    );
    const nextOrder = Number(maxRows?.[0]?.maxOrder ?? 0) + 1;

    const [result] = await db.query(
      `
        INSERT INTO etapasconversionpv (nombre, descripcion, color, sort_order, is_active)
        VALUES (?, ?, ?, ?, ?)
      `,
