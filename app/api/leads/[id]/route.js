  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT o.*, 
       ...
       WHERE o.id = ? AND o.oportunidad_id LIKE 'LD-%'`,
      [id]
    );
    ...
    const [detalles] = await db.query(
      "SELECT * FROM oportunidades_detalles WHERE oportunidad_id = ? ORDER BY created_at DESC",
      [id]
    );

    const lead = rows[0];
    lead.detalles = detalles;
...
export async function PUT(req, { params }) {
  try {
    ...
    await db.query(
      `UPDATE oportunidades_oportunidades SET ${updates.join(", ")} WHERE id = ? AND oportunidad_id LIKE 'LD-%'`,
      values
    );
...
export async function DELETE(req, { params }) {
  try {
    ...
    await db.query(
      "DELETE FROM oportunidades_oportunidades WHERE id = ? AND oportunidad_id LIKE 'LD-%'",
      [id]
    );
