...
export async function GET(req, { params }) {
  ...
  const [rows] = await db.query(
    "SELECT * FROM horaagenda_centro WHERE centro_id=? LIMIT 1",
    [centroId]
  );
...
export async function PUT(req, { params }) {
  ...
  const [rows] = await db.query(
    "SELECT id FROM horaagenda_centro WHERE centro_id=? LIMIT 1",
    [centroId]
  );

  if (rows.length) {
    await db.query(`
      UPDATE horaagenda_centro
      SET slot_minutes=?, week_json=?, updated_at=NOW()
      WHERE centro_id=?
    `, [slot_minutes, JSON.stringify(week_json), centroId]);
  } else {
    await db.query(`
      INSERT INTO horaagenda_centro(centro_id, slot_minutes, week_json, created_at, updated_at)
      VALUES(?,?,?,NOW(),NOW())
