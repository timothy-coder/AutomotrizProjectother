...
/* =========================
   ...
   - si un lead fue reprogramado, muestra solo el último
...
export async function GET(req) {
  try {
    ...
    console.error("GET /api/leadspv error:", error);
    return NextResponse.json(
      { message: "Error al listar leads", error: error.message },
      { status: 500 }
    );
  }
}
...
/* =========================
   POST: crear lead o reprogramar
   - reprogramación conserva el mismo código del padre
   - nuevos leads generan correlativo LD-[numero]
========================= */
  ...
  try {
    ...
    if (esReprogramacion) {
      ...
      if (!padreRows.length) {
        await conn.rollback();
        return NextResponse.json(
          { message: "El lead original no existe" },
          { status: 404 }
        );
      }
    ...
    await conn.rollback();
    console.error("POST /api/leadspv error:", error);
    return NextResponse.json(
      {
