...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const [existe] = await conn.query(
      `SELECT id FROM configuracion_estados_tiempopv WHERE id = ? LIMIT 1`,
      [id]
    );
    ...
    if (estado) {
      const [estadoExistente] = await conn.query(
        `SELECT id FROM configuracion_estados_tiempopv WHERE estado = ? AND id != ? LIMIT 1`,
        [estado, id]
      );
    ...
    const query = `
      UPDATE configuracion_estados_tiempopv
      SET ${updates.join(", ")}
      WHERE id = ?
    `;
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    await conn.query(
      `UPDATE configuracion_estados_tiempopv SET activo = 0 WHERE id = ?`,
      [id]
    );
